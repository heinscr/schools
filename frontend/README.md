...
Interactive choropleth map component using D3.js (NOT Leaflet):
...
- **Pure SVG**: No external map tiles or dependencies needed
...
The frontend uses a `config.json` file generated by Terraform for production, or `.env` for local development:
...
**Production:**
Config is automatically generated at `/config.json` by Terraform with API Gateway URL, Cognito settings, etc.
...
**D3.js Choropleth** was chosen over Leaflet for this application:
...
- **Offline capable**: Works without internet for map tiles

**Note**: This project does NOT use Leaflet or React-Leaflet. All mapping is pure D3.js with TopoJSON.
